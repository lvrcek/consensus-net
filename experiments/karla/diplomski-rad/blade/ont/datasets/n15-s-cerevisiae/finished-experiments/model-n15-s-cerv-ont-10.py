...
X_train = np.load('../dataset-n15-X-reshaped-train.npy')
X_validate = np.load('../dataset-n15-X-reshaped-validate.npy')
y_train = np.load('../dataset-n15-y-reshaped-train.npy')
y_validate = np.load('../dataset-n15-y-reshaped-validate.npy')
...
conv_1 = Conv2D(filters=40, kernel_size=4, padding='same', activation='relu')(input_layer)
pool_1 = MaxPool2D(pool_size=(2, 1))(conv_1)
conv_2 = Conv2D(filters=20, kernel_size=2, padding='same', activation='relu')(pool_1)
