...
def build_model_and_callbacks(suggestion):
    ...
    conv_1 = Conv1D(filters=suggestion['filters_1'], kernel_size=suggestion['kernel_size_1'], padding='same', activation='relu', kernel_regularizer=l2(suggestion['weight_regularizer']))(input_layer)
    pool_1 = MaxPooling1D(pool_size=(suggestion['pool_size_1']))(conv_1)
    conv_2 = Conv1D(filters=suggestion['filters_2'], kernel_size=suggestion['kernel_size_2'], padding='same', activation='relu', kernel_regularizer=l2(suggestion['weight_regularizer']))(pool_1)
    pool_2 = MaxPooling1D(pool_size=(suggestion['pool_size_2']))(conv_2)
    conv_3 = Conv1D(filters=suggestion['filters_3'], kernel_size=suggestion['kernel_size_3'], padding='same', activation='relu', kernel_regularizer=l2(suggestion['weight_regularizer']))(pool_2)
    bn_1 = BatchNormalization()(conv_3)

    flatten = Flatten()(bn_1)
    predictions = Dense(num_output_classes, activation='softmax', kernel_regularizer=l2(suggestion['weight_regularizer']))(flatten)
