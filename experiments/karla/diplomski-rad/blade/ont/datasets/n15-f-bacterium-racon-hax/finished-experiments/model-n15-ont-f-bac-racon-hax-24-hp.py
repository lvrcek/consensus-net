...
def build_model_and_callbacks(weight_regularizer):
    ...
    conv_1 = Conv1D(filters=16, kernel_size=4, padding='same', activation='selu', kernel_regularizer=l2(weight_regularizer))(input_layer)
    pool_1 = MaxPooling1D(pool_size=(5), strides=1)(conv_1)

    conv_2 = Conv1D(filters=32, kernel_size=4, padding='same', activation='selu', kernel_regularizer=l2(weight_regularizer))(pool_1)
    pool_2 = MaxPooling1D(pool_size=(4), strides=1)(conv_2)

    conv_3 = Conv1D(filters=48, kernel_size=4, padding='same', activation='selu', kernel_regularizer=l2(weight_regularizer))(pool_2)
...
def train_with_optimizer(suggestion, experiment):
    ...
    model, callbacks = build_model_and_callbacks(suggestion['weight_regularizer'])
